# skills — AIXSILICON Skill Repository 规划与待办（私有）
...
> [`skills/ip-development-suite/`](skills/ip-development-suite/README.md)（V1.0），
...
> 依据 [`skills/ip-development-suite/README.md`](skills/ip-development-suite/README.md)（canonical suite）
> 与 [`skill_repo_plan.md`](skill_repo_plan.md) 整理。
> 状态：`[x]` 已完成 · `[-]` 进行中 · `[ ]` 待办。
...
- [ ] 确定性 extractor 与 `aixsilicon_tool_repo` 边界落地（T1/T2，见 workflow `docs/tool-placement.md`）
