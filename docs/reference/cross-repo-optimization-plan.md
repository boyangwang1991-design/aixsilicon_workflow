...
> 历史参考：当前目标方案见 [`../architecture/target-design.md`](../architecture/target-design.md)，当前建设顺序见 [`../roadmap.md`](../roadmap.md)；本文保留原始决策背景，不维护状态。
>
> 依据：[`workflow-requirements.md`](workflow-requirements.md)（Workflow 总规划）、[`workflow-engineering-review.md`](workflow-engineering-review.md)（工程化设计参考）、
...
五类稳定契约（对应 [`workflow-requirements.md`](workflow-requirements.md:1939) §34）保持不变：Workspace / Dependency / Execution / Collaboration / Evidence。
...
[`workflow-requirements.md`](workflow-requirements.md:36) 与 [`ownership-map.yaml`](../../ownership-map.yaml:23) 内部也混用两种写法。
...
但 workflow [`workflow-requirements.md`](workflow-requirements.md:239) §4.7 已明确否决 `eda_flow_repo`/`eda_rule_repo`，且采用单一 `aixsilicon_catalog_repo` 与 `aixsilicon_techlib_repo`。
...
**决策**：workflow 采纳 tool_repo 的退出码分段，并在 `errors.py` 与 runner 中映射：设计失败（20–29）与环境失败（30–39）必须可区分，对应 [`workflow-requirements.md`](workflow-requirements.md:1626) §25 的要求。

---

## 4. 工作区本体优化（当前项目）

围绕 [`workflow-engineering-review.md`](workflow-engineering-review.md) 中的 S1–S4 结构重构，继续推进剩余闭环：
