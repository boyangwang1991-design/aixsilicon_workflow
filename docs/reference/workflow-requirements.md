...
> 历史参考：完整保留旧版总体规划中的需求、契约和验收细节；不维护当前状态。当前跨仓优先级以 [`../roadmap.md`](../roadmap.md) 为准，当前状态以 [`../progress.md`](../progress.md) 为准，Workflow 契约以 [`../workflow/`](../workflow/README.md) 为准。
...
> 本轮“扫描各仓 plan 后综合优化”的治理决议与落地结果，当前权威文件见 `docs/adr/`、`docs/workflow/release.md`、`docs/workflow/ownership.md` 和 `docs/architecture/target-design.md`；原优化稿见本目录的 `cross-repo-optimization-plan.md`。

### 35.1 治理决议（ADR-0003～0006）

| ADR | 决议 |
|---|---|
| [`0003`](../adr/0003-unified-vlnv-namespace.md) | 全组织统一 VLNV vendor 为 `aixsilicon`（`aix` 过短有歧义）；组织名仅作 URL 归属；存量 `aix:*`/`boyangwang1991-design:*` 走 deprecated 别名窗口 |
| [`0004`](../adr/0004-cli-entry-and-plugin-registry.md) | `aix` 为唯一 CLI 入口；`aixsilicon_tool_repo` 通过插件组 `aixsilicon.commands` 暴露 `aix tool`；未装时显式 `OPTIONAL_UNAVAILABLE` |
| [`0005`](../adr/0005-cross-repo-boundary-map.md) | 收敛幽灵仓：`eda-flow/eda-rules/hw-models/cbb-catalog/cbb-tech` 映射到 workflow/tool/catalog/techlib/overlay |
| [`0006`](../adr/0006-tool-ownership-and-migration.md) | 产品级确定性工具归 tool_repo；资产仓 `tools/` 仅自维护脚本；分阶段迁移 |
...
> 全文见 [`docs/plans/cross-repo-architecture-review.md`](cross-repo-architecture-review.md)。
