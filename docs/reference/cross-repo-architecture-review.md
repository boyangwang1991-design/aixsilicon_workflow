...
> 历史参考：评审结论已吸收至 [`../architecture/target-design.md`](../architecture/target-design.md) 和 ADR；本文保留原始问题编号与依据，不维护当前状态。
>
> 日期：2026-08-13
> 范围：`aixsilicon_workflow` 与 9 个资产仓的 plan 整体合理性，检查**重复构建**、**架构不合理**、**引用/被引用不清晰**。
> 依据：各仓 plan/README、[`../workflow/ownership.md`](../workflow/ownership.md)、[`docs/adr/0003-0006`](../adr/README.md)。
...
| R3 | Schema 重复维护：各仓各定义同域 Schema | workflow / hwif / dv-common / tool / catalog | **重复（已治理）** | [`../workflow/ownership.md`](../workflow/ownership.md) 单一 Owner |
