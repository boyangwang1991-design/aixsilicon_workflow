...
> 历史参考：本文保留 2026-08-13 的十仓建设全景，不维护当前优先级或状态。当前入口见 [`../index.md`](../index.md)，架构见 [`../architecture/`](../architecture/README.md)，路线图与状态见 [`../roadmap.md`](../roadmap.md) / [`../progress.md`](../progress.md)。
>
...
> 本文件保留体系建设全景；当前优先级与状态分别以 [`../roadmap.md`](../roadmap.md) 和 [`../progress.md`](../progress.md) 为准，各仓材料见 [`../index.md`](../index.md)。
...
| tools | `aixsilicon_tool_repo` | tool | P0 五包结构与 `aix tool` 插件入口已建立；tool_repo 记录 30 用例通过 | `aix wf run` 尚未转真实 provider，workspace-lock `tools:` 段与 reference 适配待完成 | 先完成 S5 端到端收尾，再扩展 socgen/ppa-bench/dv-gen |
...
| `aixsilicon_tool_repo` | 确定性生成、检查、转换、打包工具 | **私有**（交付件随公开资产仓开源） | **新增/P0** |
...
- **规划入口（尚未落地为 Flow YAML）**：`cbb-development`（`cbb.yaml` 参数契约 → 多实现 → 参数验证 → PPA）→ `cbb-verification` → `release-train`；
...
| 契约 | 内容 | 出处（归档） |
|---|---|---|
| ADR-0001 | Manifest + 独立 Clone，不用 Submodule | [`adr/0001`](../adr/0001-manifest-over-submodule.md) |
| ADR-0002 | YAML SSOT + JSON Schema | [`adr/0002`](../adr/0002-schema-driven-yaml.md) |
| ADR-0003 | VLNV 统一 `aixsilicon:*`（CLI 名保持 `aix`） | [`adr/0003`](../adr/0003-unified-vlnv-namespace.md) |
| ADR-0004 | 单入口 `aix` + 插件组 `aixsilicon.commands` | [`adr/0004`](../adr/0004-cli-entry-and-plugin-registry.md) |
| ADR-0005 | 跨仓边界映射（幽灵仓收敛） | [`adr/0005`](../adr/0005-cross-repo-boundary-map.md) |
| ADR-0006 | 工具归属与迁移（T1 入 tool_repo） | [`adr/0006`](../adr/0006-tool-ownership-and-migration.md) |
| Schema 所有权 | 每个事实域单一 Owner 仓 | [`workflow/ownership.md`](../workflow/ownership.md) |
| 成熟度映射 | `draft/qualified/proven/deprecated` 统一外部尺度 | [`workflow/release.md`](../workflow/release.md) |
| 工具归属 | T1 公共工具→tool_repo / T2 仓内脚本 / T3 私有 overlay / T4 项目脚本 | [`workflow/ownership.md`](../workflow/ownership.md) |
| 跨仓评审决议 | R1–R7（重复构建）/ A1–A4（架构）/ C1–C5（引用清晰） | [`cross-repo-architecture-review.md`](cross-repo-architecture-review.md) |
| 跨仓优化决策 | D1–D5（VLNV/仓库命名/成熟度/CLI/工具迁移） | [`cross-repo-optimization-plan.md`](cross-repo-optimization-plan.md) |
...
- **各仓 plan/todo**：见 [`index.md`](../index.md) §4
- **迁移审计**：**[`MIGRATION.md`](../MIGRATION.md)**（历史内容当前落点与人工删除检查表）
